...
function App() {
  ...
  useEffect(() => {
    if (!isViewing || !contentRef.current) return;

    // Cache headings only once when content changes to avoid repeated DOM queries
    headingsRef.current = Array.from(contentRef.current.querySelectorAll('h1, h2, h3'));

    const handleScroll = () => {
      if (scrollRafIdRef.current) return;

      // Throttle scroll updates with requestAnimationFrame for 60fps performance
      scrollRafIdRef.current = requestAnimationFrame(() => {
        ...
        // Bypassing React state for the progress bar to eliminate redundant re-renders
        ...
        if (headings.length > 0 && currentActive === '' && headings[0].getBoundingClientRect().top > triggerPoint) {
          currentActive = headings[0].id;
        }

        // Only trigger React state update when the active heading actually changes
    ...
    if (scrollContainer) {
      // Use passive: true to improve scrolling performance on mobile and desktop
